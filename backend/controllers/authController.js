...
export const registerUser = async(req, res)=>{
    try{
        ...
        if(!password || password.length <6){
            return res.status(400).send({success:false, message: 'A stronger password must be provided'})
        }
        ...
        if(!phone){
            return res.status(400).send({success:false, message: 'Please enter a valid contact number'})
        }
        if(!address){
            return res.status(400).send({success:false, message: 'Address na diley kaar baritey pathabo'})
        }
        ...
        return res.setHeader('Content-Type', 'application/json').status(200).send({success:true, message:"Successfully created the account", userInfo})
    }catch(error){
...
export const loginUser = async(req,res)=>{
    try{
        ...
        if(!matchPassword){
            return res.status(401).send({success:false, message:"Please enter a valid password"})
        }
